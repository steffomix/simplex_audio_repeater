    ...
    def audio_loop_duplex(self):
        ...
        def record_thread():
            while self.running and self.duplex_recording:
                try:
                    # Verwerfe überschüssige, alte Einträge statt zu warten - die Mic-Aufnahme
                    # darf niemals wegen einer hinterherhinkenden Wiedergabe blockieren
                    self.calculate_delayed_buffer_size()
            ...
            try:
                ...
                with self.streams_lock:
                    ...
                    if self.monitoring_enabled:
                        ...
                        self.calculate_delayed_buffer_size()
                        # Verwerfe überschüssige, alte Einträge statt zu warten - die Mic-Aufnahme
                        # darf niemals wegen einer hinterherhinkenden Monitoring-Wiedergabe blockieren
                        # (sonst läuft der ALSA-Eingangspuffer über und Samples gehen verloren)
